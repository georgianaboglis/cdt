			...
			if (ACBuilder.useArgumentFiles() && (fCmd instanceof IBuildCommand2)) {
				...
				if (argumentFileFormat != null && argumentFileFormat.contains(FILE_MACRO)) {
					try {
						...
						out.write(NLS.bind(ManagedMakeMessages.getResourceString("MakeBuilder.Using_Argument_file"), //$NON-NLS-1$
								new Object[] { argumentFile.getName(), serializedArguments }).getBytes());
						arguments = new String[] {
								argumentFileFormat.replace(FILE_MACRO, argumentFile.getAbsolutePath()) };
					} catch (IOException e) {
						ManagedBuilderCorePlugin.log(new Status(IStatus.ERROR, ManagedBuilderCorePlugin.PLUGIN_ID,
								"Error creating temp file", e)); //$NON-NLS-1$
					}
				}
			}
			try {
				fProcess = launcher.execute(fCmd.getCommand(), arguments, mapToStringArray(fCmd.getEnvironment()),
						fCmd.getCWD(), monitor);
				if (fProcess != null) {
					try {
